...
def AdminLogin(request):
    if request.method == "POST":
        ...
        # data = MycustomUser.objects.get(admin) 
        # if user.is_admin:
        user = authenticate(request, email=email, password=password)
        # user = authenticate(request,email=request.POST.get("email"), password= request.POST.get("password"))
        # print("3333",user.is_admin)
        if user is not None:
            login(request, user)
            book_list = SaveBookData.objects.all()
            return render(request, 'mainfolder/showbooks.html',{'book_list':book_list})
        else:
            return render(request, "mainfolder/login.html") 
        # else:
        #     messages.error(request, 'Admin not validate')
        #     return render(request, "mainfolder/login.html") 
    else:
        return render(request, "mainfolder/login.html")
...
def update_Book_issue_view(request,id):
    today = date.today()
    date1 = today.strftime("%d-%m-%Y")
    data=Bookissue.objects.get(pk=id)
    pemail = data.email
    tb1 = data.total_book
    bn = data.bookname
    bnobj = SaveBookData.objects.get(bookname=bn)
    tb2 = tb1 + bnobj.total_book
    # print(tb2)
    bnobj.total_book = tb2
    if data.submit == "Not submit":
        bnobj.save()
        data.submit = date1
        data.save()
    pdata=MycustomUser.objects.get(email=pemail)
...
def calculate_Book_fine_view(request,id):
    today = date.today()
